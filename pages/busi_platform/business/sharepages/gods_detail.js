 ...
 */
function discut_Countdown(tim){
	var times = tim.replace(/\-/g, "/");
    var tid = setInterval(function () {
		var totalSec=(Date.parse(times)/1000-parseInt(new Date().getTime()/1000));
        if (totalSec >= 0) {
            $("#time_new").html(getNewSyTime(totalSec));
        } else {
            clearInterval(tid);
        }
    }, 1000);
        ...
        success:function(data){
            if(data.code==1){
                ...
                if(data.info.isPromotion==0){
                    $("#present_price").html(data.info.costPrice);

                    $("#discount_type").html("");
                    $("#discount_time").html("");
                    $(".pasent_price").html("");
                }else if(data.info.isPromotion==1){
                    $("#present_price").html(data.info.discountPrice);
                    $(".pasent_price").html("");
                    ...
                    $("#discount_time").html("<p>距离恢复原价：<time id='time_new'></time></p>");
                    discut_Countdown(data.info.tag.endTime)
                }
...
window.onload=function(){
    ...
    if(isWeiXin()){
        ...
        $("#run_app").bind("click",function(){
            if (navigator.userAgent.match(/android/i)) {
                $("#run_app").attr("href","download.html");
            }
            turnApp();
        });
        $("#top_cancle").click(function(){
