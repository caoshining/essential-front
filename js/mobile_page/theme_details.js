...
$(function(){
    ...
    var num = 10;
    //brandId = 1;
    brandIntroduction(recommendId,1);
    function brandIntroduction(recommendId,counter){

        var parmaData = {
            recommendId:recommendId,//商品ID
            pageIndex:counter//页码

        };
        $.ajax({
            type: 'post',
            url: BUSINESS_URL_PRE+'/essential/recommend/queryProductDetails',
            dataType: 'json',
            data:parmaData,
            success: function(data){
                if(data.code == 1){
                    console.log(data)
                    $('title').html(data.info.recommendDetail.recommendTitle)
                    $('.head_introduction img').attr('src',data.info.recommendDetail.coverPic);
                    $('.activity_msg').html(data.info.recommendDetail.recommendDescription);
        ...
        loadDownFn : function(me){
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.code == 1){
                        //console.log(data)
                        var productList = data.info.relatedProduct;
                        var result = '';
                        counter++;
