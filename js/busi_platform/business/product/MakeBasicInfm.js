...
function GetSelt_Gui(){
    ...
    if($('.Pro_standard').length == 1){
        if($(".Product_Select>ul>li.active1").size() == []){
            ...
            var html = $("#testTrTmpl").render();
            $("#list").append(html);
            var tdTmplHTML = "<td>"+$(".Product_Select>ul>li.active1").eq(0).text()+"</td><td><input type='text'  name='pice'/></td><td><input type='text' name='repertory'/></td><td><input type='text' name='GodCode'/></td>";
            $("#list>tr").append(tdTmplHTML);
        }else{
...
//商品关键字输入
$("#PostTrade").on("keyup","#Make_keywords",function(){
